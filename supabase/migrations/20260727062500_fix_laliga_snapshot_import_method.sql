CREATE OR REPLACE FUNCTION public.replace_laliga_snapshot(p_fantasy_team_id uuid, p_league_id text, p_team_name text, p_balance numeric, p_squad_value numeric, p_squad jsonb, p_market jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
begin
  ...
  values (v_user_id, 4, true, 'official', now())
  on conflict (user_id) do update
    set current_step = 4,
        completed = true,
        selected_import_method = 'official',
        updated_at = now();
