...
export function PlayerImage({ player, size = 44 }: { player: Player; size?: number }) {
  ...
  if (!player.image || failed) {
    return (
      <span
        ...
        className="grid shrink-0 place-items-center rounded-full border border-white/10 bg-white/[.06] text-xs font-black text-[#c4b5fd]"
  ...
  return (
    <Image
      ...
      className="shrink-0 rounded-full border border-white/10 bg-white/[.04] object-contain"
    />
  );
}
