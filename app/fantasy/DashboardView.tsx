...
/*
 * `Date.now()` estaba dentro del render: las horas que faltan para el cierre se
 * recalculaban en cada re-render, que ocurre cuando a React le conviene y no
 * cuando pasa el tiempo. Dos filas podian mostrar cuentas hechas en instantes
 * distintos sin que nada hubiera cambiado.
 *
 * Se congela al montar. El contador NO va en vivo —tampoco lo hacia antes— pero
 * al menos ahora es estable y predecible. Para que corriera de verdad haria
 * falta un intervalo en la lista, no en la fila.
 */
function MarketRow({ entry }: { entry: MarketResponseLite["market"][number] }) { const [ahora] = useState(() => Date.now()); const hours = entry.expiresAt ? Math.max(0, (Date.parse(entry.expiresAt) - ahora) / 3_600_000) : null; return <article className="flex items-center gap-3 rounded-2xl bg-white/[.035] p-3"><PlayerImage player={entry.player} size={42}/><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-white">{entry.player.name}</p><p className="text-[11px] text-neutral-500">{entry.player.team} · {millions(entry.salePrice)}</p></div><div className="text-right"><p className="text-sm font-bold text-white">{hours === null ? "—" : hours < 1 ? "< 1 h" : `${Math.round(hours)} h`}</p><p className="text-[9px] text-neutral-600">para cierre</p></div></article>; }
