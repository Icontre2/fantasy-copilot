...
import { ArrowDownRight, ArrowUpRight, Banknote, BellRing, ChevronRight, Coins, ShieldAlert, ShoppingBag, Trophy, Users } from "lucide-react";
import Image from "next/image";
import type { DashboardResponse, MarketValuePoint } from "./types";
import { get, getCacheado } from "./api";
...
type IntelligenceAlert = {
  ...
  player: Player & { marketValue: number; averagePoints: number };
  ...
  return <div className="space-y-4">
    <section className="glass-strong overflow-hidden rounded-[30px] p-5 text-white">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.16em] text-[#a78bfa]">Centro de inteligencia</p><h1 className="mt-1 text-2xl font-bold tracking-tight">Lo importante de tu fantasy</h1><p className="mt-2 text-sm leading-5 text-white/55">Subidas, bajadas, cláusulas y liga. Todo lo que merece tu atención ahora.</p></div>
        <div className="rounded-2xl bg-[#7c3aed]/20 px-3 py-2 text-center ring-1 ring-[#7c3aed]/35"><Trophy size={17} className="mx-auto text-[#c4b5fd]"/><p className="mt-1 text-lg font-black">#{data.me.position ?? "—"}</p><p className="text-[10px] text-white/50">posición</p></div>
      </div>
    ...
    <section className="rounded-[28px] glass p-4">
      <div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-neutral-500">Mercado · ahora</p><h2 className="text-xl font-bold text-white">Qué está pasando</h2></div><span className="rounded-full bg-white/[.05] px-3 py-1 text-[11px] font-semibold text-neutral-400">{intelligence.length} señales</span></div>
      <div className="grid gap-2 sm:grid-cols-3">
        <Signal label="Suben" value={rising.length ? `+${rising.length}` : "—"} tone="up" icon={<ArrowUpRight size={17}/>} />
        <Signal label="Bajan" value={falling.length ? `${falling.length}` : "—"} tone="down" icon={<ArrowDownRight size={17}/>} />
        <Signal label="Cláusulas" value={clause.length ? `${clause.length}` : "—"} tone="warn" icon={<ShieldAlert size={17}/>} />
      </div>
    </section>
    ...
    <section className="rounded-[28px] border border-orange-500/20 bg-orange-500/[.06] p-4">
      ...
      <div className="mt-3 space-y-2">{clause.length ? clause.map((item) => <ClauseRow key={`${item.owner.teamId}-${item.player.id}`} item={item}/>) : <p className="rounded-2xl bg-white/[.04] p-3 text-sm text-neutral-500">No hay una cláusula urgente que vigilar.</p>}</div>
    </section>
...
function IntelligenceSection({ title, eyebrow, icon, tone, items, empty }: { title: string; eyebrow: string; icon: React.ReactNode; tone: "up" | "down"; items: IntelligenceAlert[]; empty: string }) {
  return <section className="rounded-[28px] glass p-4"><div className="flex items-center gap-3"><span className={`grid h-10 w-10 place-items-center rounded-2xl ${tone === "up" ? "bg-emerald-500/12 text-emerald-400" : "bg-rose-500/12 text-rose-400"}`}>{icon}</span><div><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-neutral-500">{eyebrow}</p><h2 className="text-xl font-bold text-white">{title}</h2></div></div><div className="mt-3 space-y-2">{items.length ? items.map((item) => <TrendRow key={`${item.owner.teamId}-${item.player.id}`} item={item} tone={tone}/>) : <p className="rounded-2xl bg-white/[.03] p-3 text-sm text-neutral-500">{empty}</p>}</div></section>;
}

function TrendRow({ item, tone }: { item: IntelligenceAlert; tone: "up" | "down" }) {
...
function ClauseRow({ item }: { item: IntelligenceAlert }) {
...
function MarketRow({ entry }: { entry: MarketResponseLite["market"][number] }) {
  const hours = entry.expiresAt ? Math.max(0, (Date.parse(entry.expiresAt) - Date.now()) / 3_600_000) : null;
...
function LeagueRow({ data, mine = false, onClick }: { data: DashboardResponse["me"] | DashboardResponse["competitors"][number]; mine?: boolean; onClick?: () => void }) {
  const content = <div className={`flex items-center gap-3 rounded-2xl p-3 ${mine ? "bg-[#7c3aed]/12 ring-1 ring-[#7c3aed]/30" : "bg-white/[.035]"}`}><Avatar name={data.manager.name} image={data.manager.avatar}/><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-white">{data.manager.name}{mine ? " · Tú" : ""}</p><p className="text-[11px] text-neutral-500">#{data.position ?? "—"} · {data.points ?? "—"} pts</p></div><div className="text-right"><p className="text-sm font-bold text-white">{millions(data.teamValue)}</p><p className="text-[9px] text-neutral-600">valor</p></div>{onClick && <ChevronRight size={15} className="text-neutral-600"/>}</div>;
  return onClick ? <button type="button" className="w-full text-left" onClick={onClick}>{content}</button> : content;
}
...
function ValueChart({ points, cargando, error, delta }: { points: MarketValuePoint[]; cargando: boolean; error: string | null; delta: number | null }) {
  if (points.length < 2) return <div className="mt-3 grid h-28 place-items-center rounded-2xl border border-dashed border-white/15 bg-white/[.03] px-6 text-center text-xs leading-5 text-white/40">{error ?? (cargando ? "Cargando cotización oficial…" : "No hay suficientes días de valor en este periodo.")}</div>;
  return <TrendChart className="mt-3" points={points.map((point) => ({ date: point.date, value: point.marketValue }))} formatValue={millions} formatDate={dayMonth} color={trendColor(delta)} label={`Valor de tu plantilla desde ${dayMonth(points[0]!.date)}, ${points.length} días`}/>;
}
...
function useSquadHistory(leagueId: string | undefined, teamId: string | undefined) {
  ...
  useEffect(() => {
    ...
    let cancelado = false;
    getCacheado<ValueHistoryResponse>(`/api/fantasy/leagues/${encodeURIComponent(leagueId)}/teams/${encodeURIComponent(teamId)}/value-history`).then((respuesta) => { if (!cancelado) { setHistories(respuesta.histories ?? {}); setError(null); } }).catch((caught: unknown) => { if (!cancelado) setError(caught instanceof Error ? caught.message : "No se pudo cargar la evolución."); }).finally(() => { if (!cancelado) setCargando(false); });
