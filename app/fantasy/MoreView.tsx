...
export function MoreView({ onSelect }: { onSelect: (section: Section) => void }) {
  return <div className="space-y-4"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-neutral-500">Herramientas</p><h2 className="text-2xl font-bold tracking-tight text-white">Todo lo demás</h2></div><div className="grid grid-cols-2 gap-3">{ITEMS.map((item) => <button key={item.id} type="button" onClick={() => onSelect(item.id)} className="rounded-[24px] border border-white/8 bg-[#121214] p-4 text-left shadow-[0_10px_35px_rgba(0,0,0,.3)] transition active:scale-[.98]"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#7c3aed]/20 text-[#a78bfa]">{item.icon}</span><p className="mt-4 font-bold text-white">{item.label}</p><p className="mt-1 text-xs leading-4 text-neutral-500">{item.description}</p></button>)}</div></div>;
}
