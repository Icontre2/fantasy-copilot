...
function PlayerProbability({ player, onSelect }: { player: LineupsResponse["teams"][number]["players"][number]; onSelect: (player: Player) => void }) {
  ...
  return <button type="button" disabled={!player.player} onClick={() => player.player && onSelect(player.player)} className="relative overflow-hidden rounded-2xl bg-[#f5f6f8] p-3 text-center transition active:scale-[.98] disabled:cursor-default" aria-label={player.player ? `Ver histórico de ${player.name}` : player.name}><div className="mx-auto grid h-16 w-16 place-items-center overflow-hidden rounded-full bg-white">{player.image ? <Image src={player.image} alt="" width={64} height={64} unoptimized className="h-full w-full object-contain"/> : <span className="text-lg font-black text-neutral-300">{player.name.slice(0,1)}</span>}</div><p className="mt-2 truncate text-sm font-semibold text-[#101a39]">{player.name}</p><p className="text-[10px] text-neutral-400">{player.position || "Jugador"}</p><span className={`mt-2 inline-block rounded-full px-2.5 py-1 text-xs font-black ${tone}`}>{player.probability}%</span></button>;
}
