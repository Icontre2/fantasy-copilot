...
export function LoginView({
  ...
  return (
    <section className="mx-auto mt-[6vh] w-full max-w-md overflow-hidden rounded-[32px] glass">
      <div className="border-b border-white/8 px-6 pb-5 pt-6">
        <div className="flex items-center gap-3">
          <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#7c3aed] text-base font-black text-white shadow-[0_0_24px_rgba(124,58,237,.28)]">LL</span>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[.16em] text-white/45">LigaLab</p>
            <h1 className="text-xl font-bold tracking-tight text-white">Iniciar sesión</h1>
          </div>
        </div>
      </div>

      {avisoDeAcceso && (
        <p className="flex gap-2 border-b border-white/8 bg-emerald-500/10 p-4 text-[12px] leading-4 text-emerald-200" role="status">
      ...
      {social && social.proveedores.length === 0 && social.motivo && (
        <div className="border-b border-white/8 px-6 py-4">
          <p className="text-[11px] leading-4 text-neutral-500">
            <strong className="text-neutral-400">Acceso social sin configurar.</strong>{" "}{social.motivo}
          </p>
        </div>
      )}
      ...
      <form onSubmit={submit} className="space-y-4 p-6">
        <label className="block">
          ...
          <span className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[.04] px-3">
            <Mail size={17} className="text-neutral-500" />
            <input type="email" required autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} className="w-full bg-transparent py-3.5 text-white outline-none" />
          </span>
        </label>
        ...
        <label className="block">
          ...
          <span className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[.04] px-3">
            ...
            <input type="password" required autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className="w-full bg-transparent py-3.5 text-white outline-none" />
          </span>
        </label>

        {aVista && <ErrorBox message={aVista} />}

        <button type="submit" disabled={busy} className="w-full rounded-2xl bg-[#7c3aed] px-4 py-4 font-bold text-white shadow-lg disabled:opacity-50">
          {busy ? "Entrando…" : social?.identificado ? "Conectar mi cuenta de LALIGA" : "Entrar"}
        </button>

        <button
          type="button"
          onClick={() => { setShowSocialToken((current) => !current); setTokenError(null); }}
        ...
        >
          <KeyRound size={17} /> Mi cuenta de LALIGA usa Google, Apple o Facebook
        </button>
        ...
        {showSocialToken && (
          <div className="space-y-3 rounded-2xl border border-[#7c3aed]/30 bg-[#7c3aed]/10 p-4">
            <div>
              ...
              <p className="mt-1 text-[11px] leading-4 text-neutral-300">Inicia sesión en la web oficial y pega aquí la respuesta de token que LALIGA entrega.</p>
            </div>
            <a href="https://miliga.laliga.com/" target="_blank" rel="noreferrer" className="flex w-full items-center justify-center gap-2 rounded-xl bg-white px-3 py-2.5 text-xs font-bold text-[#1f1f1f]">
              Abrir acceso oficial de LALIGA <ExternalLink size={14} />
            </a>
            <label className="block">
              <span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wide text-neutral-400">Respuesta JSON de LALIGA</span>
              <textarea value={tokenInput} onChange={(event) => setTokenInput(event.target.value)} spellCheck={false} autoCapitalize="off" autoCorrect="off" placeholder={'{"access_token":"…","refresh_token":"…"}'} className="min-h-28 w-full resize-y rounded-xl border border-white/10 bg-black/25 p-3 font-mono text-[11px] leading-4 text-white outline-none focus:border-[#8b5cf6]" />
            </label>
            {tokenError && <ErrorBox message={tokenError} />}
            <button type="button" onClick={submitImportedToken} disabled={tokenBusy || !tokenInput.trim()} className="w-full rounded-xl bg-[#7c3aed] px-4 py-3 text-sm font-bold text-white disabled:opacity-50">
        ...
        <p className="flex gap-2 rounded-2xl bg-emerald-500/10 p-3 text-[11px] leading-4 text-emerald-300">
          <ShieldCheck size={16} className="mt-px shrink-0" />
          <span>La contraseña se usa una vez y no se guarda.</span>
        </p>
        ...
        <p className="text-center text-[11px] leading-4 text-neutral-500">
          <a href="/privacidad" className="underline underline-offset-2 hover:text-neutral-300">Qué datos maneja LigaLab</a>{" · "}Herramienta independiente, no afiliada a LALIGA.
        </p>
      </form>
    </section>
  );
...
function BotonProveedor({ proveedor }: { proveedor: Proveedor }) {
  ...
  return (
    <a href={`/api/fantasy/auth/social/start?provider=${proveedor}`} className={`flex min-h-[52px] w-full items-center justify-center gap-3 rounded-2xl px-4 font-bold ${estilo[proveedor]}`}>
      {logo[proveedor]} Entrar con {NOMBRES_PROVEEDOR[proveedor]}
    </a>
  );
}
