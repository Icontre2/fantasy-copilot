...
export function AlertsView({ data, onChanged }: { data: AlertsResponse; onChanged: () => void }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 [scrollbar-width:none]" aria-label="Filtrar alertas">{FILTERS.map((option) => <button key={option.id} type="button" onClick={() => setFilter(option.id)} className={`min-h-11 shrink-0 rounded-2xl px-4 text-sm font-bold ${filter === option.id ? "bg-[#7c3aed] text-white" : "border border-white/10 bg-[#121214] text-neutral-500"}`}>{option.label}</button>)}</div>
