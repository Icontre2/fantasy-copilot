...
function AlertCard({ alert, mine, cash, busy, onSelect, onBuyout }: { alert: ClauseAlert; mine: boolean; cash: number | null; busy: boolean; onSelect: (player: Player) => void; onBuyout: (alert: ClauseAlert) => void }) {
  ...
  return <article className="rounded-[26px] border border-white/10 bg-[#0d0f14] p-4 shadow-[0_16px_40px_rgba(0,0,0,.18)]">
    ...
    <ClauseRiskLine value={alert.official.marketValue} clause={alert.official.buyoutClause} />
    ...
    {!mine && <button type="button" disabled={disabled} onClick={() => onBuyout(alert)} className="mt-3 min-h-11 w-full rounded-2xl border border-rose-500/40 bg-rose-500/10 px-4 text-sm font-bold text-rose-300 transition enabled:hover:bg-rose-500/15 disabled:border-white/5 disabled:bg-white/5 disabled:text-neutral-600">{busy ? "Confirmando…" : alert.official.isShielded ? "Cláusula bloqueada" : cannotAfford ? "Caja insuficiente" : `Cláusulazo · ${millions(alert.official.buyoutClause)}`}</button>}
  </article>;
}

function ClauseRiskLine({ value, clause }: { value: number; clause: number }) {
  const covered = clause > 0 ? Math.max(0, Math.min(1, value / clause)) : 0;
  return (
    <div className="mt-3" aria-label={`El valor cubre ${Math.round(covered * 100)}% de la cláusula`}>
      <div className="h-1.5 overflow-hidden rounded-full bg-white/[.06]">
        <div className="h-full rounded-full bg-gradient-to-r from-rose-600 via-rose-500 to-red-400 shadow-[0_0_14px_rgba(244,63,94,.5)]" style={{ width: `${Math.max(3, covered * 100)}%` }} />
      </div>
    </div>
  );
}
